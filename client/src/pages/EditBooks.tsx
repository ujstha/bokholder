...
import { update, fetch } from "../utilities/httpUtils";
import { cloudUpload } from "ionicons/icons";
import "../theme/add-books.css";
import { RouteComponentProps } from "react-router";
import moment from "moment";

const EditBooks: React.FC<RouteComponentProps<{ id: string }>> = ({
  match
}) => {
  ...
  const [posterRes, setPosterRes] = useState("");
  ...
  function editBooks() {
    ...
    return update(match.params.id, formData).then(res => {
      setIsLoading(false);
      setResColor("success");
      ...
      <IonHeader>
        <IonToolbar>
          <IonButtons slot="start">
            <IonBackButton defaultHref={`/page/books/${match.params.id}`} />
          </IonButtons>
          <IonTitle>Edit Books</IonTitle>
        </IonToolbar>
      </IonHeader>
            ...
            >
              ...
              <Form.Item label="Released" className="m-0">
                <DatePicker
                  ...
                  value={moment(released, "YYYY-MM-DD")}
              ...
              <Form.Item label="Genre of book" className="m-0">
                <Select
                  ...
                  onChange={(value: never[]) => setGenre(value)}
                  value={genre}
                  showArrow={true}
                  showSearch={true}
                >
              ...
              <Form.Item label="Description of book" className="">
                <Input.TextArea
                  rows={5}
              ...
              <Form.Item>
                <Button
                  ...
                  type="ghost"
                  onClick={() =>
                    (document.location.href = `/page/books/${match.params.id}`)
                  }
                >
                  <i className="fa fa-close"></i> &ensp;Cancel
                </Button>
                <Button
                  htmlType="submit"
                  size="large"
                ...
                >
                  <i className="fa fa-edit"></i> &ensp; Edit this Book
                </Button>
              </Form.Item>
            </Form>
          </div>
